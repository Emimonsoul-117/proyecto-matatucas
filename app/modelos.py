...
class Ejercicio(bd.Model):
    ...
    tipo = bd.Column(
        bd.Enum(
            'opcion_multiple', 'verdadero_falso', 'numerico',
            'completar_texto', 'ordenar_pasos', 'respuesta_corta',
        ),
        nullable=False,
    )
...
def generar_numero_control_personalizado(email):
    """Genera un número de control personalizado basado en el correo, garantizando que sea único."""
    parte_local = email.split('@')[0].upper()
    # Limitar a 20 caracteres (límite de VARCHAR(20) en numero_control)
    control = parte_local[:20]
    
    # Si ya es único, retornarlo
    if not Usuario.query.filter_by(numero_control=control).first():
        return control
        
    # De lo contrario, resolver colisión agregando un número incremental al final
    base_control = control
    secuencia = 1
    while True:
        sufijo = f"_{secuencia}"
        max_base_len = 20 - len(sufijo)
        control = f"{base_control[:max_base_len]}{sufijo}"
        if not Usuario.query.filter_by(numero_control=control).first():
            return control
        secuencia += 1


def migrar_usuarios_sin_numero_control():
    """Asigna un número de control personalizado a todos los usuarios que no tengan uno."""
    usuarios_sin_control = Usuario.query.filter((Usuario.numero_control == None) | (Usuario.numero_control == '')).all()
    for usuario in usuarios_sin_control:
        control = generar_numero_control_personalizado(usuario.email)
        usuario.numero_control = control
        bd.session.commit()
