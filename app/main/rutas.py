...
# ==============================================================================
# GAMIFICACIÓN: RANKING MUNDIAL (FASE 3)
# ==============================================================================

@main.route('/ranking')
@login_required
def ranking():
    from ..modelos import Estudiante, Usuario, ConfiguracionUsuario
    
    # Obtener configuración del usuario actual
    config_actual = ConfiguracionUsuario.query.get(current_user.id)
    ocultar_me = config_actual.ocultar_ranking if config_actual else False
    
    # Query para Top 50 estudiantes basado en puntos (descendente)
    # Excluir a los que hayan marcado "ocultar_ranking" en Configuración
    top_estudiantes = (
        bd.session.query(Usuario, Estudiante)
        .join(Estudiante, Usuario.id == Estudiante.id_usuario)
        .outerjoin(ConfiguracionUsuario, Usuario.id == ConfiguracionUsuario.id_usuario)
        .filter(bd.or_(ConfiguracionUsuario.ocultar_ranking == False, ConfiguracionUsuario.ocultar_ranking == None))
        .order_by(Estudiante.puntos_totales.desc(), Usuario.nombre.asc())
        .limit(50)
        .all()
    )
    
    # Buscar el puesto del usuario actual si es estudiante y está participando
    mi_ranking = None
    if current_user.rol == 'estudiante' and not ocultar_me:
        puntos_actual = Estudiante.query.get(current_user.id).puntos_totales
        # Calcular posición cuenta cuántos tienen estrictamente más puntos
        posicion = (
            bd.session.query(bd.func.count(Estudiante.id_usuario))
            .outerjoin(ConfiguracionUsuario, Estudiante.id_usuario == ConfiguracionUsuario.id_usuario)
            .filter(bd.or_(ConfiguracionUsuario.ocultar_ranking == False, ConfiguracionUsuario.ocultar_ranking == None))
            .filter(Estudiante.puntos_totales > puntos_actual)
            .scalar()
        )
        mi_ranking = posicion + 1
        
    return render_template('main/ranking.html', 
                           top_estudiantes=top_estudiantes, 
                           mi_ranking=mi_ranking,
                           ocultar_me=ocultar_me)
